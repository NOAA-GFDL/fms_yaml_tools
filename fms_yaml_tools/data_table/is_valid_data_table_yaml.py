...
def check_gridname(grid_name):
    ...
    if (grid_name not in valid):
        raise Exception(grid_name + ' is not a valid gridname. The only values allowed are "OCN", "LND", "ATM", "ICE"')


def check_fieldname_code(fieldname):
    if (fieldname == ""):
        raise Exception("Fieldname can't be empty")


def check_filename_and_field(field, interp_method):
    if (field == "" and interp_method != ""):
        raise Exception('If "fieldname_file" is empty, interp_method must be empty')


def check_interp_method(interp_method):
    """Check if the interp method is valid. Crashes if it not. """
    valid = ["bilinear", "bicubic", "none"]
    if (interp_method not in valid):
        raise Exception(interp_method +
                        ' is not a valid interp_method. ' +
                        'The only values allowed are "bilinear", "bicubic", and "none"')
...
def check_region_type(region_type):
    ...
    if (region_type not in valid):
        raise Exception(region_type +
                        'is not a valid region_type. ' +
                        'The only values allowed are "inside_region" and "outside_region"')


def check_if_bounds_present(entry):
    """Check if the region bounds are valid, crashes if they are not """
    if ("lat_start" not in entry):
        raise Exception('lat_start must be present if region_type is set')
    if ("lat_end" not in entry):
        raise Exception('lat_end must be present if region_type is set')
    if ("lon_start" not in entry):
        raise Exception('lon_start must be present if region_type is set')
    if ("lon_end" not in entry):
        raise Exception('lon_end must be present if region_type is set')


def check_region(my_type, start, end):
    """Check if the region is defined correctly. Crashes if it not. """
    if (start > end):
        raise Exception(my_type + "_start is greater than " + my_type + "_end")
