...
def check_freq(file_name, freq, freq_units):
    ...
    if (freq < -1):
        sys.exit('ERROR: freq needs to greater than -1. Check your freq entry for file:' + file_name)
...
def check_required_diag_files_key(diag_file):
    ...
    if 'freq' not in diag_file:
        sys.exit('ERROR: freq is a required key! Add it for file:' + diag_file['file_name'])
    if 'freq_units' not in diag_file:
        sys.exit('ERROR: freq_units is a required key! Add it for file:' + diag_file['file_name'])
    if 'time_units' not in diag_file:
        sys.exit('ERROR: time_units is a required key! Add it for file:' + diag_file['file_name'])
    if 'unlimdim' not in diag_file:
        sys.exit('ERROR: unlimdim is a required key! Add it for file:' + diag_file['file_name'])
...
def check_new_file_freq(diag_file):
    ...
    if 'new_file_freq' in diag_file:
        if 'new_file_freq_units' not in diag_file:
            sys.exit('ERROR: new_file_freq is present, but not new_file_freq_units. Check you entry for file:'
                     + diag_file['file_name'])
        if (diag_file['new_file_freq'] < 1):
            sys.exit('ERROR: new_file_freq needs to be greater than 0. Check your new_file_freq for '
                     + diag_file['file_name'])
        ...
        if 'new_file_freq_units' in diag_file:
            sys.exit('ERROR: new_file_freq_units is present, but not new_file_freq. Check your entry for file:'
                     + diag_file['file_name'])
...
def check_file_duration(diag_file):
    ...
    if 'file_duration' in diag_file:
        if 'file_duration_units' not in diag_file:
            sys.exit('ERROR: file_duration is present, but not file_duration_units. Check you entry for file:'
                     + diag_file['file_name'])
        if (diag_file['file_duration'] < 1):
            sys.exit('ERROR: file_duration_units needs to be greater than 0. Check your file_duration_units for '
                     + diag_file['file_name'])
        ...
        if 'file_duration_units' in diag_file:
            sys.exit('ERROR: file_duration_units is present, but not file_duration. Check your entry for file:'
                     + diag_file['file_name'])
...
def check_start_time(diag_file):
    ...
    if 'file_duration' not in diag_file:
        sys.exit('ERROR: file_duration is needed if start_time is present. Check your entry for file:'
                 + diag_file['file_name'])
...
def check_sub_region(diag_file):
    ...
    if sub_region['grid_type'] not in valid:
        sys.exit('ERROR: the grid_type (' + sub_region['grid_type'] + ') and file:'
                 + diag_file['file_name'] + ' is not valid')

    if 'dim1_begin' in sub_region and 'dim1_end' in sub_region:
        if sub_region['dim1_begin'] > sub_region['dim1_end']:
            sys.exit('ERROR: dim1_begin in your subregion of file:'
                     + diag_file['file_name'] + ' is greater than dim1_end')

    if 'dim2_begin' in sub_region and 'dim2_end' in sub_region:
        if sub_region['dim2_begin'] > sub_region['dim2_end']:
            sys.exit('ERROR: dim2_begin in your subregion of file:'
                     + diag_file['file_name'] + ' is greater than dim2_end')

    if 'dim3_begin' in sub_region and 'dim3_end' in sub_region:
        if sub_region['dim3_begin'] > sub_region['dim3_end']:
            sys.exit('ERROR: dim3_begin in your subregion of file:'
                     + diag_file['file_name'] + ' is greater than dim3_end')

    if 'dim4_begin' in sub_region and 'dim4_end' in sub_region:
        if sub_region['dim4_begin'] > sub_region['dim4_end']:
            sys.exit('ERROR: dim4_begin in your subregion of file:'
                     + diag_file['file_name'] + ' is greater than dim4_end')
...
def check_required_diag_field_key(diag_field, file_name):
    ...
    if 'module' not in diag_field:
        sys.exit('ERROR: module is a required field! Add it for variable:'
                 + diag_field['var_name'] + ' in file: ' + file_name)
    if 'reduction' not in diag_field:
        sys.exit('ERROR: reduction is a required field! Add it for variable:'
                 + diag_field['var_name'] + ' in file: ' + file_name)
    if 'kind' not in diag_field:
        sys.exit('ERROR: kind is a required field! Add it for variable:'
                 + diag_field['var_name'] + ' in file: ' + file_name)
...
def check_reduction(diag_field, file_name):
    ...
    if "diurnal" in reduction:
        if int(reduction[7:9]) < 0:
            sys.exit('ERROR: The number of diurnal samples in ' + reduction + ' for variable:'
                     + diag_field['var_name'] + ' and file:' + file_name + ' is not valid')
    elif "pow" in reduction:
        if int(reduction[3:5]) < 0:
            sys.exit('ERROR: The power value in ' + reduction + ' for variable:'
                     + diag_field['var_name'] + ' and file:' + file_name + ' is not valid')
    elif reduction not in valid:
        sys.exit('ERROR: The reduction (' + reduction + ') in variable:' + diag_field['var_name'] + ' and file:'
...
def check_kind(diag_field, file_name):
    ...
    if diag_field['kind'] not in valid:
        sys.exit('ERROR: The kind (' + diag_field['kind'] + ') in variable:'
                 + diag_field['var_name'] + ' and file:' + file_name + ' is not valid')
