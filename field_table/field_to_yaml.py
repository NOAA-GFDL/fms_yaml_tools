...
class Field:
  def __init__(self, in_field_type, entry_tuple):
    ...
    for in_prop in entry_tuple[1]:
      if 'species' in self.field_type:
        self.process_species(in_prop)
      elif 'tracer' in self.field_type:
  ...
  def process_species(self, prop):
    ...
    if len(comma_split) > 1:
      ...
      for idx, sub_param in enumerate(eq_splits):
        if len(sub_param) < 2:
          eq_splits[idx-1][1] += f',{sub_param[0]}'
          eq_splits.pop(idx)
...
with open('field_table', 'r') as fh:
  whole_file = fh.read()
# Eliminate spaces and quotes
...
# Make out_yaml file
with open('field_table.yaml', 'w') as yaml_file:
